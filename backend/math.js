...
const WILD_SYMBOLS = ["WILD_TROPHY"];
const BASE_WEIGHTS = {
    "10": 108,
    J: 101,
    Q: 94,
...
const RTP_DAMPENER = { 96: 1, 94: 0.94, 92: 0.88, 88: 0.76 };
...
export class SlotMath {
    ...
    spin(request) {
        ...
        const spinZones = mode === "base" ? new Set() : persistentZones;
        ...
        for (let index = 0; index < 12; index += 1) {
            ...
            const goldenAdded = Array.from(removed).map(parseKey).filter(Boolean);
            ...
            cascades.push({
                ...
                removed: Array.from(removed).map(parseKey).filter(Boolean),
                goldenAdded,
                clusters,
        ...
        const activatorLanded = request.forceActivator === true ||
            mode === "worldFinal" ||
    ...
    simulate(spins, bet = 1) {
        let totalBet = 0;
        let totalWin = 0;
        let hits = 0;
        let maxWinMultiplier = 0;
        const bonusTriggers = { extraTime: 0, penaltyShootout: 0, worldFinal: 0 };
        for (let i = 0; i < spins; i += 1) {
            ...
            if (result.triggeredBonus)
                bonusTriggers[result.triggeredBonus.mode] += 1;
        }
    ...
    findClusters(grid, cascadeMultiplier) {
        const wins = [];
        ...
        for (const symbol of REGULAR_SYMBOLS) {
            ...
            for (let row = 0; row < ROWS; row += 1) {
                for (let col = 0; col < COLS; col += 1) {
                    ...
                    if (visited.has(key))
                        continue;
                    if (grid[row][col].id !== symbol && !WILD_SYMBOLS.includes(grid[row][col].id))
                        continue;
                    ...
                    if (cells.length >= 5 && naturalCells.length > 0) {
                        ...
                        if (claimed.has(`${symbol}:${uniqueKey}`))
                            continue;
    ...
    collectRemovedCells(grid, clusters) {
        const removed = new Set();
        for (const cluster of clusters) {
            for (const cell of cluster.cells) {
                const id = grid[cell.row][cell.col].id;
                if (id === cluster.symbol || WILD_SYMBOLS.includes(id))
                    removed.add(positionKey(cell));
            }
        }
        return removed;
    }
    cascadeGrid(grid, removed, mode, request) {
        const next = Array.from({ length: ROWS }, () => Array.from({ length: COLS }, () => ({ id: "10" })));
        for (let col = 0; col < COLS; col += 1) {
            const survivors = [];
            ...
            while (writeRow >= 0)
                next[writeRow--][col] = { id: this.randomSymbol(mode, request) };
        }
        return next;
    }
    activateGoldenZones(zones, mode) {
        const positions = Array.from(zones).map(parseKey).filter(Boolean);
        const reveals = [];
        const captains = [];
        const collectors = [];
        let total = 0;
    ...
    pickCoinTier(mode) {
        ...
        if (this.rng.chance(goldChance))
            return "gold";
        if (this.rng.chance(silverChance))
            return "silver";
        return "bronze";
    }
    pickCoinValue(tier, mode) {
    ...
    getBonusTrigger(mode, scatterCount) {
        if (mode !== "base")
            return undefined;
        if (scatterCount >= 5)
            return { mode: "worldFinal", spins: 15, scatterCount, upgraded: true };
        if (scatterCount >= 4)
            return { mode: "penaltyShootout", spins: 12, scatterCount };
        if (scatterCount >= 3)
            return { mode: "extraTime", spins: 8, scatterCount };
        return undefined;
    }
    getRetriggerSpins(mode, scatterCount) {
...
function getClusterPay(symbol, size) {
...
function floodFill(grid, start, symbol, visited) {
    const result = [];
    const queue = [start];
    while (queue.length) {
        ...
        if (visited.has(key))
            continue;
        if (current.col < 0 || current.col >= COLS || current.row < 0 || current.row >= ROWS)
            continue;
        const id = grid[current.row][current.col].id;
        if (id !== symbol && !WILD_SYMBOLS.includes(id))
            continue;
        visited.add(key);
        result.push(current);
...
function containsSymbol(grid, symbol) {
