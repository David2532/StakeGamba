...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Production build                         | PASS — deterministic pre-clean Vite static build plus exact commit/tree/clean identity on run `33483678009`, exit 0, run BS-20260901-11 |
| Lint                                     | PASS — app lint on exact run `33483678009`, exit 0, run BS-20260901-11 |
| Typecheck                                | PASS — production source/config `svelte-check`, `checkJs: true`, 0 errors / 0 warnings on exact run `33483678009`, exit 0 |
| Automated app tests                      | PASS — 123/123, exit 0, exact run `33483678009`, BS-20260901-11; includes the compositor-pulse regression plus prior package/accessibility/provider/gameplay coverage |
| Math/package invariants                  | PASS — 7/7 math gates and 300,000 books; exact identity-bound package generation/readback PASS; fingerprint `d03fab…78d8`; exact run `33483678009` |
| Current-head browser/E2E                 | PASS — clean exact commit `cae8c0b`; isolated-package Chromium passed 71/71 scenarios plus 1854/1854 checks; exact build tree `9d699302…cd9`; unexpected network/page/request failures 0 |
| Desktop/portrait/landscape visual review | PASS for current surfaces — exact-package geometry from artifact `9791347108` passed at 1920x1080, 1366x768, 390x844, 360x740, 844x390, 768x1024 and Replay 360x640 plus the orientation round trip. The active BLACKOUT transition, 1920, 390, 844 and Replay captures were inspected; dimming/lock geometry, penguin/vault pixels, named square boards, centered values and >=44px controls remain visible without overlap, document scroll or crop regression; screen-reader/physical-device sign-off remains open |
...
| Animation/cinematics                     | PARTIAL — authoritative spin/reveal/feature anticipation, BLACKOUT entry/exit, cascades, semantic Vaultkeeper reactions and seven-column stops pass normal/turbo/reduced, skip, cleanup and exact-payout proof. Moving the large environment pulse from filters to opacity improves the same BLACKOUT scenario from 50.0ms to 33.4ms p95 and from four to one >50ms frame; max stays 66.7ms. Approved Spine clips and real-device pacing remain |
