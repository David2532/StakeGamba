...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Production build                         | PASS — Vite static build on exact run `33439810616`, exit 0, run BS-20260831-29 |
| Lint                                     | PASS — app lint on exact run `33439810616`, exit 0, run BS-20260831-29 |
| Typecheck                                | PASS — production source/config `svelte-check`, `checkJs: true`, 0 errors / 0 warnings on exact run `33439810616`, exit 0 |
| Automated app tests                      | PASS — 108/108, exit 0, exact run `33439810616`, BS-20260831-29; includes fail-closed binding for expired-session reauthentication plus canonical mode wins and live feature lifecycles |
| Math/package invariants                  | PASS — 7/7 math gates and 300,000 books; exact package generation/readback PASS; fingerprint `d03fab…78d8`; exact run `33439810616` |
| Current-head browser/E2E                 | PASS — clean exact commit `5767f33`; isolated-package Chromium passed 62/62 scenarios plus 1518/1518 checks; `ERR_SESSION` never auto-retries, explicit reload reauthenticates once, and only a fresh player action sends the next play; unexpected network/page/request failures 0 |
| Desktop/portrait/landscape visual review | PASS for current surfaces — exact-package expired-session failure/recovery screenshots and geometry from artifact `9776234530` inspected at 1280x720, 1920x1080, 390x844, 844x390 and Replay 360x640. Error/recovery controls, penguin/vault pixels, square boards and values remain visible without overlap, scroll or crop regression; prior 1366x768 evidence remains source-identical and manual/device sign-off remains open |
| Live wallet/replay/restore               | PARTIAL — fractional new-play/restore, uncertain paid-play reload/restore, auth-503 and expired-session explicit reload/re-auth, failed-settlement and `ERR_IPB` explicit restore with no automatic or duplicate write, competing-input deduplication, exact currency families and natural/confirmed feature plays are current-package browser proven; broader provider evidence remains |
