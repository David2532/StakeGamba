...
async function waitForAssetPaint(page, timeoutMs = 10_000) {
	await page.waitForFunction(
		() => {
			...
			return (
				['painted', 'fallback', 'omitted'].includes(characterState ?? '') &&
	...
	assert.ok(
		['painted', 'fallback', 'omitted'].includes(states.character),
...
async function runNetworkScenarios(browser, origin) {
	...
	await runScenario('navigation-teardown-aborts-auth-without-wallet-write', async (record) => {
		const group = 'navigation-teardown-aborts-auth-without-wallet-write';
		const context = await browser.newContext({ viewport: { width: 1280, height: 720 } });
		try {
			await context.addInitScript(
				({ rgsOrigin }) => {
					const auditKey = 'blacksite-qa-auth-teardown-audit';
					const nativeFetch = window.fetch.bind(window);
					const readAudit = () => {
						try {
							return (
					...
					window.fetch = (input, init = {}) => {
						...
						return new Promise((_resolve, reject) => {
							...
							const abort = () => {
								...
								nextAudit.aborts += 1;
								writeAudit(nextAudit);
								reject(new DOMException('BlackSite QA teardown aborted transport.', 'AbortError'));
							};
							if (signal?.aborted) abort();
							else signal?.addEventListener('abort', abort, { once: true });
						});
			...
			await page.waitForFunction(() => {
				const audit = JSON.parse(
					sessionStorage.getItem('blacksite-qa-auth-teardown-audit') ?? 'null',
				);
			...
			const teardownAudit = await page.evaluate(() =>
				JSON.parse(sessionStorage.getItem('blacksite-qa-auth-teardown-audit') ?? 'null'),
			);
			assertExactRequest(group, network.byEndpoint.authenticate[0], {
				method: 'POST',
				path: '/wallet/authenticate',
			...
			check(
				group,
				'navigation teardown aborts the pending app-owned RGS transport',
				teardownAudit?.attempts === 2 && teardownAudit?.aborts === 1,
				serialize(teardownAudit),
			);
			check(
				group,
				'teardown recovery performs one successful authenticate and zero wallet writes',
				network.byEndpoint.authenticate.length === 1 &&
					network.byEndpoint.play.length === 0 &&
					network.byEndpoint.endRound.length === 0 &&
					network.byEndpoint.event.length === 0,
			...
			check(
				group,
				'reload after teardown returns to authoritative ready',
				(await runtimeState(page)) === 'live-ready' &&
					!(await page.locator(SELECTORS.primaryAction).isDisabled()),
	...
	await runScenario('audio-policy-cues-mute-and-persistence', async (record) => {
		...
		try {
			await context.addInitScript(() => {
				...
				window.__setBlacksiteDocumentHidden = (nextHidden) => {
					hidden = nextHidden;
		...
		try {
			...
			const playReveal = async (profile) => {
				...
				const { timeline, lossTimeline } = await page.evaluate(() => ({
					...
					lossTimeline: window.__blacksiteLossTimeline,
				}));
				return {
					timeline,
					window: revealWindow(timeline),
					lossTimeline,
					lossWindow: lossWindow(lossTimeline),
					screenshot,
					lossScreenshot,
				};
			};
	...
	await runScenario('active-feature-restore-resumes-at-checkpoint-once', async (record) => {
		...
		try {
			await context.addInitScript(() => {
				window.__blacksiteRestoreTrace = [];
				const capture = () => {
					const state =
						document.documentElement.dataset.runtimeState ??
						document.body?.dataset.runtimeState ??
					...
					const snapshot = {
						...
						board,
						finalWin:
							document.querySelector('[data-testid="final-win"]')?.textContent?.trim() ?? null,
						mode: document.querySelector('.mode-readout')?.textContent?.trim() ?? null,
						phase:
							document.querySelector('[data-testid="board"]')?.getAttribute('data-motion-phase') ??
							null,
					};
					const signature = JSON.stringify(snapshot);
					const trace = window.__blacksiteRestoreTrace;
					if (trace.at(-1)?.signature === signature) return;
			...
			const restoreTrace = await page.evaluate(() =>
				window.__blacksiteRestoreTrace.map(({ signature: _signature, ...snapshot }) => snapshot),
			);
			const restoring = restoreTrace.find(
				(snapshot) =>
					snapshot.state === 'live-restoring' &&
					snapshot.finalWin === '—' &&
					snapshot.board.length === 49 &&
					snapshot.board.some((symbol) => symbol !== '') &&
					/BREACH RUN/i.test(snapshot.mode ?? ''),
			);
			check(
				group,
			...
			check(
				group,
				'restore sends zero duplicate plays and settles the active round exactly once',
			...
			check(
				group,
				'restore never rewrites the already-persisted checkpoint',
				actualCheckpointCursors.every(
					(cursor) => cursor !== encodePresentationCursor(resumeEventIndex),
				),
				serialize(actualCheckpointCursors),
			);
			for (const request of network.byEndpoint.event) {
			...
			check(
				group,
				'restored feature completes to the exact authoritative payout, balance and ready shell',
				completed.state === 'live-ready' &&
					completed.phase === 'idle' &&
					completed.character === 'idle_a' &&
					completed.boardCells === 49 &&
			...
			record.restore = {
				...
				actualCheckpointCursors,
				restoring,
				traceStates: restoreTrace.map(({ state }) => state),
				completed,
			};
			record.screenshot = await saveScreenshot(page, group);
			record.network = network;
	...
	return page.evaluate((selectors) => {
		...
		return {
			...
			vaultkeeper: {
				...
				imageExists: Boolean(vaultkeeperImage),
				...
				resourceCount: performance
					...
					.filter((entry) =>
						entry.name.endsWith('/assets/blacksite/character/penguin-vaultkeeper-fallback-v1.webp'),
					).length,
			},
			environment: {
...
function assertGeometryRecord(group, audit, viewport) {
	...
	check(
		group,
		`vaultkeeper fallback is ${expectsVaultkeeper ? 'visible and loaded once' : 'compact-omitted'}`,
			...
			(expectsVaultkeeper
				? Boolean(
						audit.vaultkeeper.imageExists &&
							...
							audit.vaultkeeper.resourceCount === 1,
					)
				: !audit.vaultkeeper.imageExists &&
					audit.vaultkeeper.naturalWidth === 0 &&
					audit.vaultkeeper.paintState === 'omitted' &&
...
async function runGeometryScenarios(browser, origin) {
	...
	await runScenario('vaultkeeper-responsive-resource-gate', async (record) => {
		...
		try {
			...
			await page.waitForFunction(
				() =>
					document
						.querySelector('[data-testid="vaultkeeper-presence"]')
						?.getAttribute('data-asset-paint-state') === 'painted',
			);
			const desktopAudit = await geometryAudit(page);
			...
			await page.waitForFunction(
				() =>
					document
						.querySelector('[data-testid="vaultkeeper-presence"]')
						?.getAttribute('data-asset-paint-state') === 'omitted' &&
					!document.querySelector('[data-testid="vaultkeeper-presence"] img'),
			);
			await waitForAssetPaint(page);
			const compactAfter = await geometryAudit(page);
			check(
				group,
				'compact first paint requests no character; desktop transition requests it once; compact return removes the decoded element without another request',
				compactBefore.vaultkeeper.resourceCount === 0 &&
					!compactBefore.vaultkeeper.imageExists &&
					desktopAudit.vaultkeeper.resourceCount === 1 &&
					desktopAudit.vaultkeeper.imageExists &&
					compactAfter.vaultkeeper.resourceCount === 1 &&
					!compactAfter.vaultkeeper.imageExists &&
					compactAfter.vaultkeeper.paintState === 'omitted',
				serialize({ compactBefore, desktopAudit, compactAfter }),
			);
			check(
				group,
			...
			assertCleanNetwork(group, network);
			assertCleanDiagnostics(group, diagnostics);
			record.assets = { compactBefore, desktopAudit, compactAfter };
			record.network = network;
			record.diagnostics = diagnostics;
		} finally {
			await context.close();
