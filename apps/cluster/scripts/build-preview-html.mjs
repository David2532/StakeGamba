...
const extraCss = `
	...
	.cell img.dropping { animation: drop-in 0.42s cubic-bezier(.34,.86,.4,1) both; }
	.cell img.clearing { animation: clear-out 0.26s ease-in forwards; }
	.stage.turbo .cell img.dropping { animation-duration: 0.2s; }
	.stage.turbo .cell img.clearing { animation-duration: 0.12s; }
	@keyframes drop-in { 0% { transform: translateY(-360%) scale(0.82); opacity: 0; }
		56% { opacity: 1; }
		74% { transform: translateY(7%) scaleX(1.08) scaleY(0.9); opacity: 1; }
		88% { transform: translateY(-3%) scaleX(0.97) scaleY(1.05); }
